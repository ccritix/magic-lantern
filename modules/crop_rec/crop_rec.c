...
static int is_supported_mode()
{
    ...
    if (!is_movie_mode() || CROP_PRESET_MENU == CROP_PRESET_OFF)
        return 0;

    /* workaround getting below cams working with focus aid */
    static int last_hs_aid = 0;
    if (!get_halfshutter_pressed())
        last_hs_aid = get_ms_clock();
    if (get_ms_clock() - last_hs_aid > 300 && get_halfshutter_pressed() && (is_6D || is_5D3) && !RECORDING && (zoomaid == 0x1 || zoomaid == 0x2))
        return 0;
...
static inline int get_video_mode_index()
{
    ...
    if (video_mode_crop)
    {
        ...
        return (video_mode_fps == 24) ? 6 : (video_mode_fps == 25) ? 7 : (video_mode_fps == 30) ? 8 : (video_mode_fps == 50) ? 9 :
                                                                                                                             /* (video_mode_fps == 60) */ 10;
    }

    /* regular video modes */
    return (video_mode_fps == 24) ? 0 : (video_mode_fps == 25) ? 1 : (video_mode_fps == 30) ? 2 : (video_mode_fps == 50) ? 3 :
                                                                                                                         /* (video_mode_fps == 60) */ 4;
}
...
static inline void FAST calc_skip_offsets(int *p_skip_left, int *p_skip_right, int *p_skip_top, int *p_skip_bottom)
{
    ...
    if (p_skip_left)
        *p_skip_left = skip_left + reg_skip_left;
    if (p_skip_right)
        *p_skip_right = skip_right + reg_skip_right;
    if (p_skip_top)
        *p_skip_top = skip_top + reg_skip_top;
    if (p_skip_bottom)
        *p_skip_bottom = skip_bottom + reg_skip_bottom;
}
...
static int get_top_bar_adjustment()
{
    ...
    case CROP_PRESET_3x3_1X_45p:
        if (is_720p())
            return 28; /* 0x1D0017 from 0x10017 */
    /* fall through */
    default:
 ...
 */
    ...
    if (is_5D3)
    {
        ...
        case CROP_PRESET_3X_TALL:
            cmos_new[1] = /* vertical centering (trial and error) */
                (video_mode_fps == 24) ? PACK12(8, 13) : (video_mode_fps == 25) ? PACK12(8, 12) : (video_mode_fps == 30) ? PACK12(9, 11) : (video_mode_fps == 50) ? PACK12(12, 10) : (video_mode_fps == 60) ? PACK12(13, 10) : (uint32_t)-1;
        ...
        case CROP_PRESET_3x3_1X_45p:
            if (is_720p())
            {
                /* start/stop scanning line, very large increments */
                cmos_new[1] =
                    ((crop_preset == CROP_PRESET_3x3_1X_48p || crop_preset == CROP_PRESET_3x3_1X_45p) && (x3crop == 0x0 && !crop_patch)) ? PACK12(3, 15) : ((x3crop == 0x1 || crop_patch) && crop_preset == CROP_PRESET_3x3_1X_50p) ? PACK12(14, 11) : ((x3crop == 0x1 || crop_patch) && crop_preset == CROP_PRESET_3x3_1X_60p) ? PACK12(15, 11) : ((x3crop == 0x1 || crop_patch) && (crop_preset == CROP_PRESET_3x3_1X_48p || crop_preset == CROP_PRESET_3x3_1X_45p)) ? PACK12(11, 10) : (video_mode_fps == 50 || crop_preset == CROP_PRESET_3x3_1X_50p) ? PACK12(4, 14) : (video_mode_fps == 60 || crop_preset == CROP_PRESET_3x3_1X_60p) ? PACK12(6, 14) : (uint32_t)-1;
        ...
        case CROP_PRESET_3K:
            cmos_new[1] =                                                                                                                                                                                                      /* vertical centering (trial and error) */
                (video_mode_fps == 24) ? PACK12(8, 12) : (video_mode_fps == 25) ? PACK12(8, 12) : (video_mode_fps == 30) ? PACK12(9, 11) : (video_mode_fps == 50) ? PACK12(13, 10) : (video_mode_fps == 60) ? PACK12(14, 10) : /* 13,10 has better centering, but overflows */
                                                                                                                                                                                         (uint32_t)-1;
        ...
        case CROP_PRESET_UHD:
            cmos_new[1] =
                (video_mode_fps == 24) ? PACK12(4, 9) : (video_mode_fps == 25) ? PACK12(4, 9) : (video_mode_fps == 30) ? PACK12(5, 8) : (video_mode_fps == 50) ? PACK12(12, 9) : (video_mode_fps == 60) ? PACK12(13, 9) : (uint32_t)-1;
        ...
        case CROP_PRESET_4K_HFPS:
            cmos_new[1] =
                (video_mode_fps == 24) ? PACK12(4, 15) : (video_mode_fps == 25) ? PACK12(4, 15) : (video_mode_fps == 30) ? PACK12(6, 14) : (video_mode_fps == 50) ? PACK12(10, 11) : (video_mode_fps == 60) ? PACK12(12, 11) : (uint32_t)-1;
        ...
        case CROP_PRESET_x10_EOSM:
            ...
            if ((isoauto == 0x1 || isoauto == 0x2 || isoauto == 0x3) && lens_info.raw_iso == 0x0 && HDR_iso_a == 0x0)
            {
                if (isoauto == 0x1 && lens_info.raw_iso_auto > 0x54)
                    cmos_new[0] = 0x84b; // stick to iso 400
                if (isoauto == 0x2 && lens_info.raw_iso_auto > 0x5c)
                    cmos_new[0] = 0x86f; // stick to iso 800
                if (isoauto == 0x3 && lens_info.raw_iso_auto > 0x63)
                    cmos_new[0] = 0x893; // stick to iso 1600
            }
            break;
    ...
    if ((isoauto == 0x1 || isoauto == 0x2 || isoauto == 0x3) && (lens_info.raw_iso == 0x0 && HDR_iso_a == 0x0))
    {
        ...
        if (isoauto == 0x2 && lens_info.raw_iso_auto > 0x5c)
            EngDrvOutLV(0xC0F0b12c, 0x8);
        if (isoauto == 0x3 && lens_info.raw_iso_auto > 0x63)
            EngDrvOutLV(0xC0F0b12c, 0x9);

        if ((!is_5D3 && !is_6D) && isoauto == 0x1 && lens_info.raw_iso_auto > 0x54)
            cmos_new[0] = 0x84b; // stick to iso 400
        if ((!is_5D3 && !is_6D) && isoauto == 0x2 && lens_info.raw_iso_auto > 0x5c)
            cmos_new[0] = 0x86f; // stick to iso 800
        if ((!is_5D3 && !is_6D) && isoauto == 0x3 && lens_info.raw_iso_auto > 0x63)
            cmos_new[0] = 0x893; // stick to iso 1600

        if (is_5D3 && isoauto == 0x1 && lens_info.raw_iso_auto > 0x54)
            cmos_new[0] = 0x223; // stick to iso 400
        if (is_5D3 && isoauto == 0x2 && lens_info.raw_iso_auto > 0x5c)
            cmos_new[0] = 0x333; // stick to iso 800
        if (is_5D3 && isoauto == 0x3 && lens_info.raw_iso_auto > 0x63)
            cmos_new[0] = 0x443; // stick to iso 1600

        if (is_6D && isoauto == 0x1 && lens_info.raw_iso_auto > 0x54)
            cmos_new[0] = 0x22; // stick to iso 400
        if (is_6D && isoauto == 0x2 && lens_info.raw_iso_auto > 0x5c)
            cmos_new[0] = 0x33; // stick to iso 800
        if (is_6D && isoauto == 0x3 && lens_info.raw_iso_auto > 0x63)
            cmos_new[0] = 0x44; // stick to iso 1600
    }
 ...
 * Workaround: let's call it here. */
    ...
    if (shutter_blanking)
    {
        ...
        if (is_100D)
        {
            if (slowshutter == 0x0 || (crop_preset != CROP_PRESET_4K_100D && slowshutter == 0x1))
                shutter_blanking = adjust_shutter_blanking(shutter_blanking);
        }

        if (is_EOSM)
        {
            if (slowshutter == 0x0 && crop_preset == CROP_PRESET_4K_EOSM && timelapse != 0x0)
                shutter_blanking = adjust_shutter_blanking(shutter_blanking);
            if (crop_preset == CROP_PRESET_4K_EOSM && timelapse == 0x0)
                shutter_blanking = adjust_shutter_blanking(shutter_blanking);
            if (crop_preset != CROP_PRESET_4K_EOSM)
                shutter_blanking = adjust_shutter_blanking(shutter_blanking);
        }
    }
...
static inline uint32_t reg_override_bits(uint32_t reg, uint32_t old_val)
{
    ...
    {
        /* 100D is a buggy mf! */
        if (!get_halfshutter_pressed() && !is_100D)
        ...
        if (get_ms_clock() - last_hs_unpress > 200 && get_halfshutter_pressed())
        {
            ...
            if (!is_100D && (get_ms_clock() - last_hs_unpress < 1500))
    ...
    if (!RECORDING && (is_EOSM || is_100D || is_6D || is_5D3))
    {
        ...
        if (bitdepth == 0x0 && (is_EOSM || is_100D || is_6D || is_5D3))
        {
            ...
            case 0xc0f0815c:
                return (timelapse != 0x0 && slowshutter == 0x1 && (crop_preset == CROP_PRESET_4K_100D || crop_preset == CROP_PRESET_4K_EOSM)) ? 0x7 : 0x2;
            }
        }
    }
    ...
    if (is_EOSM)
    {
        ...
        case 0xC0F06800:
            return 0x10010 + reg_6800_width + (reg_6800_height << 16);
        }
...
static inline uint32_t reg_override_3X_tall(uint32_t reg, uint32_t old_val)
{
    ...
    if (video_mode_fps >= 50)
    {
        ...
        int timerB =
            (video_mode_fps == 50) ? 1200 : (video_mode_fps == 60) ? 1001 : -1;
 ...
 * pushing the resolution a tiny bit further */
    int head_adj =
        (video_mode_fps == 50) ? -30 : (video_mode_fps == 60) ? -20 : 0;
...
static inline uint32_t reg_override_3x3_50p(uint32_t reg, uint32_t old_val)
{
    ...
    if (lv_dispsize != 1)
        *(volatile uint32_t *)0xC0F06014 = 0x613;
...
static inline uint32_t reg_override_3K(uint32_t reg, uint32_t old_val)
{
    ...
    if (video_mode_fps != 25 && video_mode_fps != 50)
    {
        int timerA = 455;
        int timerB =
            (video_mode_fps == 24) ? 2200 : (video_mode_fps == 30) ? 1760 : (video_mode_fps == 60) ? 880 : -1;
...
static inline uint32_t reg_override_4K_hfps(uint32_t reg, uint32_t old_val)
{
    ...
    int timerB =
        (video_mode_fps == 24) ? 3422 : (video_mode_fps == 25) ? 3282 : (video_mode_fps == 30) ? 2766 : (video_mode_fps == 50) ? 1658 : (video_mode_fps == 60) ? 1383 : -1;
...
static inline uint32_t reg_override_UHD(uint32_t reg, uint32_t old_val)
{
    ...
    int timerA =
        (video_mode_fps == 25) ? 547 : (video_mode_fps == 50) ? 546 : 550;
    int timerB =
        (video_mode_fps == 24) ? 1820 : (video_mode_fps == 25) ? 1755 : (video_mode_fps == 30) ? 1456 : (video_mode_fps == 50) ? 879 : (video_mode_fps == 60) ? 728 : -1;
...
static inline uint32_t reg_override_3xcropmode_100d(uint32_t reg, uint32_t old_val)
{
    if (is_1080p())
    {
        ...
        {
        case 0xC0F06804:
            return 0x4a701d7 + reg_6804_width + (reg_6804_height << 16);
        }
    }
    ...
    if (is_720p())
    {
        ...
        {
        case 0xC0F06804:
            return 0x2d801d7 + reg_6804_width + (reg_6804_height << 16);
        }
    }
    ...
    {
    case 0xC0F06014:
        return set_25fps == 0x1 ? 0x89e + reg_6014 : 0x8a1 + reg_6014;
    case 0xC0F0600c:
        return set_25fps == 0x1 ? 0x25b025b - 24 + reg_6008 + (reg_6008 << 16) : 0x25b025b + reg_6008 + (reg_6008 << 16);
    case 0xC0F06008:
        return set_25fps == 0x1 ? 0x25b025b - 24 + reg_6008 + (reg_6008 << 16) : 0x25b025b + reg_6008 + (reg_6008 << 16);
    case 0xC0F06010:
        return set_25fps == 0x1 ? 0x25b - 24 + reg_6008 : 0x25b + reg_6008;
    }
...
static inline uint32_t reg_override_2K_100d(uint32_t reg, uint32_t old_val)
{
    ...
    /* X: (3072+140)/8 + 0x17, adjusted for 3072 in raw_rec */
    case 0xC0F06804:
        return 0x5ac02a1 + reg_6804_width + (reg_6804_height << 16); // 2520x1418  x5 Mode;

    case 0xC0F06014:
        return set_25fps == 0x1 ? 0x6d5 + reg_6014 : 0x71e + reg_6014;
    case 0xC0F0713c:
        return 0x5ac + reg_713c;
    case 0xC0F07150:
        return 0x58c + reg_7150;
    }
...
static inline uint32_t reg_override_3K_100d(uint32_t reg, uint32_t old_val)
{
    ...
    {
    case 0xC0F06804:
        return 0x5b90319 + reg_6804_width + (reg_6804_height << 16); // 3000x1432 24fps x5 Mode;
    ...
    case 0xC0F06010:
        return 0x34b + reg_6008;
    case 0xC0F06008:
        return 0x34b034b + reg_6008 + (reg_6008 << 16);
    case 0xC0F0600C:
        return 0x34b034b + reg_6008 + (reg_6008 << 16);

    case 0xC0F06014:
        return 0x62c + reg_6014;
    case 0xC0F0713c:
        return 0x5ba + reg_713c;
    }
...
static inline uint32_t reg_override_4K_100d(uint32_t reg, uint32_t old_val)
{
    ...
    /* X: (3072+140)/8 + 0x17, adjusted for 3072 in raw_rec */
    case 0xC0F06804:
        return ((timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) && ratios == 0x0) ? 0xbd904ff + reg_6804_width + (reg_6804_height << 16) : ((timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) && ratios == 0x1) ? 0x83504ff + reg_6804_width + (reg_6804_height << 16) : ((timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) && ratios == 0x2) ? 0x85904ff + reg_6804_width + (reg_6804_height << 16) : ((timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) && ratios == 0x3) ? 0xaff04ff + reg_6804_width + (reg_6804_height << 16) : ((timelapse == 0x1 || timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x4 || timelapse == 05 || timelapse == 0x6) && ratios == 0x0) ? 0xbd90427 + reg_6804_width + (reg_6804_height << 16) : ((timelapse == 0x1 || timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x4 || timelapse == 05 || timelapse == 0x6) && ratios == 0x1) ? 0x6cb0427 + reg_6804_width + (reg_6804_height << 16) : ((timelapse == 0x1 || timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x4 || timelapse == 05 || timelapse == 0x6) && ratios == 0x2) ? 0x6e90427 + reg_6804_width + (reg_6804_height << 16) : ((timelapse == 0x1 || timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x4 || timelapse == 05 || timelapse == 0x6) && ratios == 0x3) ? 0x9170427 + reg_6804_width + (reg_6804_height << 16) : (ratios == 0x1) ? 0x6cb0427 : (ratios == 0x2) ? 0x6e90427 : (ratios == 0x3) ? 0x9170427 : 0xbd90427;

    case 0xC0F06824:
        return (timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) ? 0x56a : 0x4ca;
    case 0xC0F06828:
        return (timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) ? 0x56a : 0x4ca;
    case 0xC0F0682C:
        return (timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) ? 0x56a : 0x4ca;
    ...
    case 0xC0F06010:
        return (timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) ? 0x57b : 0x45b + reg_6008;
    case 0xC0F06008:
        return (timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) ? 0x57b057b + reg_6008 + (reg_6008 << 16) : 0x45b045b + reg_6008 + (reg_6008 << 16);
    case 0xC0F0600C:
        return (timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) ? 0x57b057b + reg_6008 + (reg_6008 << 16) : 0x45b045b + reg_6008 + (reg_6008 << 16);

    case 0xC0F06014:
        return (RECORDING && timelapse == 0x1) ? 0xffff : (RECORDING && timelapse == 0x7) ? 0xffff : (RECORDING && timelapse == 0x2) ? 0x6ff9 : (RECORDING && timelapse == 0x8) ? 0x58f7 : (RECORDING && timelapse == 0x3) ? 0x37ff : (RECORDING && timelapse == 0x9) ? 0x2c7f : (RECORDING && timelapse == 0x4) ? 0x2553 : (RECORDING && timelapse == 0x5) ? 0x1bfe : (RECORDING && timelapse == 0x6) ? 0x1665 : (timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) ? 0xcb7 + reg_6014 : 0xfff + reg_6014;

    case 0xC0F0713c:
        return ((timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) && ratios == 0x0) ? 0xbd9 + reg_713c : ((timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) && ratios == 0x1) ? 0x835 + reg_713c : ((timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) && ratios == 0x2) ? 0x859 + reg_713c : ((timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) && ratios == 0x3) ? 0xaff + reg_713c : ((timelapse == 0x1 || timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x4 || timelapse == 05 || timelapse == 0x6) && ratios == 0x0) ? 0xbd9 + reg_713c : ((timelapse == 0x1 || timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x4 || timelapse == 05 || timelapse == 0x6) && ratios == 0x1) ? 0x6cb + reg_713c : ((timelapse == 0x1 || timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x4 || timelapse == 05 || timelapse == 0x6) && ratios == 0x2) ? 0x6e9 + reg_713c : ((timelapse == 0x1 || timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x4 || timelapse == 05 || timelapse == 0x6) && ratios == 0x3) ? 0x917 + reg_713c : (ratios == 0x1) ? 0x6cb : (ratios == 0x2) ? 0x6e9 : (ratios == 0x3) ? 0x917 : 0xbd9;
    }

    /* 4k timelapse function. For slowshutter this gives no first dark frame */
    if (RECORDING && timelapse == 0x1 && slowshutter == 0x1)
        *(volatile uint32_t *)0xC0F06014 = 0xffff;
    if (RECORDING && timelapse == 0x2 && slowshutter == 0x1)
        *(volatile uint32_t *)0xC0F06014 = 0x6ff9;
    if (RECORDING && timelapse == 0x3 && slowshutter == 0x1)
        *(volatile uint32_t *)0xC0F06014 = 0x37ff;
    if (RECORDING && timelapse == 0x4 && slowshutter == 0x1)
        *(volatile uint32_t *)0xC0F06014 = 0x2553;
    if (RECORDING && timelapse == 0x5 && slowshutter == 0x1)
        *(volatile uint32_t *)0xC0F06014 = 0x1bfe;
    if (RECORDING && timelapse == 0x6 && slowshutter == 0x1)
        *(volatile uint32_t *)0xC0F06014 = 0x1665;

    /* 4k timelapse function. For slowshutter this gives no first dark frame */
    if (RECORDING && timelapse == 0x7 && slowshutter == 0x1)
        *(volatile uint32_t *)0xC0F06014 = 0xffff;
    if (RECORDING && timelapse == 0x8 && slowshutter == 0x1)
        *(volatile uint32_t *)0xC0F06014 = 0x58f7;
    if (RECORDING && timelapse == 0x9 && slowshutter == 0x1)
        *(volatile uint32_t *)0xC0F06014 = 0x2c7f;

    /* set fps flag for mlv_lite.c */
    if (timelapse == 0x1)
        EngDrvOutLV(0xc0f0501c, 0x20);
    if (timelapse == 0x2)
        EngDrvOutLV(0xc0f0501c, 0x21);
    if (timelapse == 0x3)
        EngDrvOutLV(0xc0f0501c, 0x22);
    if (timelapse == 0x4)
        EngDrvOutLV(0xc0f0501c, 0x23);
    if (timelapse == 0x5)
        EngDrvOutLV(0xc0f0501c, 0x24);
    if (timelapse == 0x6)
        EngDrvOutLV(0xc0f0501c, 0x25);
    if (timelapse == 0x7)
        EngDrvOutLV(0xc0f0501c, 0x20);
    if (timelapse == 0x8)
        EngDrvOutLV(0xc0f0501c, 0x21);
    ...
    if (!RECORDING && timelapse != 0x0 && slowshutter == 0x1)
    {
        ...
        {
        case 0xC0F42744:
            return (timelapse == 0x1 || timelapse == 0x7) ? 0x4040404 : (timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x8) ? 0x3030303 : (timelapse == 0x4 || timelapse == 0x5 || timelapse == 0x6 || timelapse == 0x9) ? 0x2020202 : 0x1010101;
        }
    }
...
static inline uint32_t reg_override_3x1_mv720_50fps_100d(uint32_t reg, uint32_t old_val)
{
    ...
    case 0xC0F06804:
        return 0x2d801d7 + reg_6804_width + (reg_6804_height << 16);
    case 0xC0F0713c:
        return 0x305 + reg_713c;
    case 0xC0F07150:
        return 0x300 + reg_7150;

    /* 50 fps */
    case 0xC0F06014:
        return 0x4bb + reg_6014;
    case 0xC0F0600c:
        return 0x20f020f + reg_6008 + (reg_6008 << 16);
    case 0xC0F06008:
        return 0x20f020f + reg_6008 + (reg_6008 << 16);
    case 0xC0F06010:
        return 0x20f + reg_6008;

    case 0xC0F06824:
        return 0x206 + reg_6824;
    case 0xC0F06828:
        return 0x206 + reg_6824;
    case 0xC0F0682c:
        return 0x206 + reg_6824;
...
static inline uint32_t reg_override_4K_3x1_100D(uint32_t reg, uint32_t old_val)
{
    ...
    {
    case 0xC0F06804:
        return 0x3100413 + reg_6804_width + (reg_6804_height << 16);
    case 0xC0F06824:
        return 0x4ca;
    case 0xC0F06828:
        return 0x4ca;
    ...
    case 0xC0F06830:
        return 0x4ca;
    case 0xC0F06010:
        return 0x45f + reg_6008;
    case 0xC0F06008:
        return 0x45f050f + reg_6008 + (reg_6008 << 16);
    case 0xC0F0600C:
        return 0x45f045f + reg_6008 + (reg_6008 << 16);
    case 0xC0F06014:
        return 0x405 + reg_6014;
    case 0xC0F0713c:
        return 0x310 + reg_713c;
    case 0xC0F07150:
        return 0x305 + reg_7150;
    }
...
static inline uint32_t reg_override_5K_3x1_100D(uint32_t reg, uint32_t old_val)
{
    ...
    {
    case 0xC0F06804:
        return 0x2e7050f + reg_6804_width + (reg_6804_height << 16);
    case 0xC0F06824:
        return 0x56a;
    case 0xC0F06828:
        return 0x56a;
    ...
    case 0xC0F06830:
        return 0x56a;
    case 0xC0F06010:
        return 0x57b + reg_6008;
    case 0xC0F06008:
        return 0x57b057b + reg_6008 + (reg_6008 << 16);
    case 0xC0F0600C:
        return 0x57b057b + reg_6008 + (reg_6008 << 16);
    case 0xC0F06014:
        return 0x3b5 + reg_6014;
    case 0xC0F0713c:
        return 0x2e8 + reg_713c;
    case 0xC0F07150:
        return 0x2e2 + reg_7150;
    }
...
static inline uint32_t reg_override_1080p_100d(uint32_t reg, uint32_t old_val)
{
    ...
    {
    case 0xC0F06804:
        return 0x45902a1 + reg_6804_width + (reg_6804_height << 16);
    case 0xC0F0713c:
        return 0x459 + reg_713c;
    case 0xC0F07150:
        return 0x450 + reg_7150;

    case 0xC0F06014:
        return set_25fps == 0x1 ? 0x5ec + reg_6014 : 0x62b + reg_6014;
    case 0xC0F06010:
        return 0x34b + reg_6008;
    case 0xC0F06008:
        return 0x34b034b + reg_6008 + (reg_6008 << 16);
...
static inline uint32_t reg_override_1x3_100d(uint32_t reg, uint32_t old_val)
{
    ...
    {
    case 0xC0F06804:
        return 0x4c301d7 + reg_6804_width + (reg_6804_height << 16);

    case 0xC0F06014:
        return 0x9df + reg_6014;
    case 0xC0F0600c:
        return 0x20f020f + reg_6008 + (reg_6008 << 16);
    case 0xC0F06008:
        return 0x20f020f + reg_6008 + (reg_6008 << 16);
...
static inline uint32_t reg_override_anamorphic_rewired_100d(uint32_t reg, uint32_t old_val)
{
    ...
    if ((ratios != 0x1) && (ratios != 0x2) && (ratios != 0x3))
    {
        ...
        {
        case 0xC0F06804:
            return 0x79f01ed + reg_6804_width + (reg_6804_height << 16);
        ...
        case 0xC0F07150:
            return 0x36c + reg_7150;

        case 0xC0F06824:
            return 0x722 + reg_6824;
        case 0xC0F06828:
            return 0x722 + reg_6824;
        case 0xC0F0682C:
            return 0x722 + reg_6824;
    ...
    if (ratios == 0x1 || ratios == 0x2)
    {
        ...
        {
        case 0xC0F06804:
            return 0x77701ed + reg_6804_width + (reg_6804_height << 16);
...
static inline uint32_t reg_override_center_z_eosm(uint32_t reg, uint32_t old_val)
{
    ...
    {
    case 0xC0F06804:
        return 0x4550298 + reg_6804_width + (reg_6804_height << 16); /* 2520x1072  x5 Mode; */
    }
    ...
    {
    case 0xC0F0713c:
        return 0x455 + reg_713c;
    case 0xC0F07150:
        return 0x428 + reg_7150;
    case 0xC0F06014:
        return set_25fps == 0x1 ? 0x747 - 76 + reg_6014 : 0x747 + reg_6014;
...
static inline uint32_t reg_override_2K_eosm(uint32_t reg, uint32_t old_val)
{
    ...
    if (ratios == 0x1 || ratios == 0x2)
    {
        ...
        {
        case 0xC0F06804:
            return 0x44c0298 + reg_6804_width + (reg_6804_height << 16); /* 2520x1072  x5 Mode; */

        case 0xC0F0713c:
            return 0x44c + reg_713c;
        case 0xC0F07150:
            return 0x435 + reg_7150;
        case 0xC0F06014:
            return set_25fps == 0x1 ? 0x747 - 76 + reg_6014 : 0x747 + reg_6014;
    ...
    else
    {
        ...
        {
        case 0xC0F06804:
            return 0x5a70298 + reg_6804_width + (reg_6804_height << 16); /* 2520x1418  x5 Mode; */
        case 0xC0F0713c:
            return 0x5a7 + reg_713c;
        case 0xC0F07150:
            return 0x5a0 + reg_7150;
        case 0xC0F06014:
            return set_25fps == 0x1 ? 0x747 - 76 + reg_6014 : 0x747 + reg_6014;
...
static inline uint32_t reg_override_3K_eosm(uint32_t reg, uint32_t old_val)
{
    ...
    if (ratios == 0x0)
    {
        ...
        {
        case 0xC0F06804:
            return 0x5b90318 + reg_6804_width + (reg_6804_height << 16); // 3032x1436  x5 Mode;
        case 0xC0F06014:
            return (get_halfshutter_pressed() && (zoomaid == 0x1 || zoomaid == 0x2) && !RECORDING) ? 0x839 : 0x62c + reg_6014;
        case 0xC0F0713c:
            return 0x5b9 + reg_713c;
        case 0xC0F06824:
            return 0x3ca;
        case 0xC0F06828:
            return 0x3ca;
    ...
    if (ratios == 0x1 || ratios == 0x2)
    {
        ...
        case 0xC0F06804:
            return 0x5190310 + reg_6804_width + (reg_6804_height << 16);
        case 0xC0F0713c:
            return 0x519 + reg_713c;
        case 0xC0F07150:
            return 0x514 + reg_7150;
        case 0xC0F06014:
            return 0x839 + reg_6014;
        case 0xC0F06824:
            return 0x3ca;
        case 0xC0F06828:
            return 0x3ca;
    ...
    if (ratios == 0x3)
    {
        ...
        {
        case 0xC0F06804:
            return 0x6b50310 + reg_6804_width + (reg_6804_height << 16);
        case 0xC0F0713c:
            return 0x6b5 + reg_713c;
        case 0xC0F06824:
            return 0x4ca;
        case 0xC0F06828:
            return 0x4ca;
        ...
        case 0xC0F06830:
            return 0x4ca;
        case 0xC0F06014:
            return 0xc71 + reg_6014;
        case 0xC0F06010:
            return 0x45b + reg_6008;
        case 0xC0F06008:
            return 0x45b045b + reg_6008 + (reg_6008 << 16);
        case 0xC0F0600C:
            return 0x45b045b + reg_6008 + (reg_6008 << 16);
        }
    }
...
static inline uint32_t reg_override_4K_eosm(uint32_t reg, uint32_t old_val)
{
    ...
    {
    case 0xC0F06804:
        return ((timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) && ratios == 0x0) ? 0xbd704fe + reg_6804_width + (reg_6804_height << 16) : ((timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) && ratios == 0x1) ? 0x84104fe + reg_6804_width + (reg_6804_height << 16) : ((timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) && ratios == 0x2) ? 0x86504fe + reg_6804_width + (reg_6804_height << 16) : ((timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) && ratios == 0x3) ? 0xb0f04fe + reg_6804_width + (reg_6804_height << 16) : ((timelapse == 0x1 || timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x4 || timelapse == 05 || timelapse == 0x6) && ratios == 0x0) ? 0xbd7041e + reg_6804_width + (reg_6804_height << 16) : ((timelapse == 0x1 || timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x4 || timelapse == 05 || timelapse == 0x6) && ratios == 0x1) ? 0x6cb041e + reg_6804_width + (reg_6804_height << 16) : ((timelapse == 0x1 || timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x4 || timelapse == 05 || timelapse == 0x6) && ratios == 0x2) ? 0x6e9041e + reg_6804_width + (reg_6804_height << 16) : ((timelapse == 0x1 || timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x4 || timelapse == 05 || timelapse == 0x6) && ratios == 0x3) ? 0x917041e + reg_6804_width + (reg_6804_height << 16) : (ratios == 0x1) ? 0x6cb041e + reg_6804_width + (reg_6804_height << 16) : (ratios == 0x2) ? 0x6e9041e + reg_6804_width + (reg_6804_height << 16) : (ratios == 0x3) ? 0x917041e + reg_6804_width + (reg_6804_height << 16) : 0xbd7041e + reg_6804_width + (reg_6804_height << 16);
    ...
    case 0xC0F06010:
        return (timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) ? 0x56b : 0x45b + reg_6008;
    case 0xC0F06008:
        return (timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) ? 0x56b056b + reg_6008 + (reg_6008 << 16) : 0x45b045b + reg_6008 + (reg_6008 << 16);
    case 0xC0F0600C:
        return (timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) ? 0x56b056b + reg_6008 + (reg_6008 << 16) : 0x45b045b + reg_6008 + (reg_6008 << 16);

    case 0xC0F06014:
        return ((RECORDING && timelapse == 0x1 && slowshutter == 0x1) || (timelapse == 0x1 && slowshutter == 0x0)) ? 0xffff : ((RECORDING && timelapse == 0x2 && slowshutter == 0x1) || (timelapse == 0x2 && slowshutter == 0x0)) ? 0x6ff9 : ((RECORDING && timelapse == 0x3 && slowshutter == 0x1) || (timelapse == 0x3 && slowshutter == 0x0)) ? 0x37ff : ((RECORDING && timelapse == 0x4 && slowshutter == 0x1) || (timelapse == 0x4 && slowshutter == 0x0)) ? 0x2553 : ((RECORDING && timelapse == 0x5 && slowshutter == 0x1) || (timelapse == 0x5 && slowshutter == 0x0)) ? 0x1bfe : ((RECORDING && timelapse == 0x6 && slowshutter == 0x1) || (timelapse == 0x6 && slowshutter == 0x0)) ? 0x1665 : ((RECORDING && timelapse == 0x7 && slowshutter == 0x1) || (timelapse == 0x7 && slowshutter == 0x0)) ? 0xffff : ((RECORDING && timelapse == 0x8 && slowshutter == 0x1) || (timelapse == 0x8 && slowshutter == 0x0)) ? 0x58f7 : ((RECORDING && timelapse == 0x9 && slowshutter == 0x1) || (timelapse == 0x9 && slowshutter == 0x0)) ? 0x2c7f : (timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) ? 0xcb7 + reg_6014 : (ratios == 0x1 || ratios == 0x2) ? 0xc71 + reg_6014 : (ratios == 0x0) ? 0x1665 + reg_6014 : 0xfff + reg_6014;

    case 0xC0F0713c:
        return ((timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) && ratios == 0x0) ? 0xbd7 + reg_713c : ((timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) && ratios == 0x1) ? 0x841 + reg_713c : ((timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) && ratios == 0x2) ? 0x865 + reg_713c : ((timelapse == 0x7 || timelapse == 0x8 || timelapse == 0x9) && ratios == 0x3) ? 0xb0f + reg_713c : ((timelapse == 0x1 || timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x4 || timelapse == 05 || timelapse == 0x6) && ratios == 0x0) ? 0xbd7 + reg_713c : ((timelapse == 0x1 || timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x4 || timelapse == 05 || timelapse == 0x6) && ratios == 0x1) ? 0x6cb + reg_713c : ((timelapse == 0x1 || timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x4 || timelapse == 05 || timelapse == 0x6) && ratios == 0x2) ? 0x6e9 + reg_713c : ((timelapse == 0x1 || timelapse == 0x2 || timelapse == 0x3 || timelapse == 0x4 || timelapse == 05 || timelapse == 0x6) && ratios == 0x3) ? 0x917 + reg_713c : (ratios == 0x1) ? 0x6cb : (ratios == 0x2) ? 0x6e9 : (ratios == 0x3) ? 0x917 : 0xbd7;
    }
    ...
    /* set fps flag for mlv_lite.c */
    if (timelapse == 0x1 && slowshutter == 0x1)
        EngDrvOutLV(0xc0f0501c, 0x20);
    if (timelapse == 0x2 && slowshutter == 0x1)
        EngDrvOutLV(0xc0f0501c, 0x21);
    if (timelapse == 0x3 && slowshutter == 0x1)
        EngDrvOutLV(0xc0f0501c, 0x22);
    if (timelapse == 0x4 && slowshutter == 0x1)
        EngDrvOutLV(0xc0f0501c, 0x23);
    if (timelapse == 0x5 && slowshutter == 0x1)
        EngDrvOutLV(0xc0f0501c, 0x24);
    if (timelapse == 0x6 && slowshutter == 0x1)
        EngDrvOutLV(0xc0f0501c, 0x25);
    if (timelapse == 0x7 && slowshutter == 0x1)
        EngDrvOutLV(0xc0f0501c, 0x20);
    if (timelapse == 0x8 && slowshutter == 0x1)
        EngDrvOutLV(0xc0f0501c, 0x21);
    if (timelapse == 0x9 && slowshutter == 0x1)
        EngDrvOutLV(0xc0f0501c, 0x22);
...
static inline uint32_t reg_override_4K_3x1_EOSM(uint32_t reg, uint32_t old_val)
{
    ...
    {
    case 0xC0F06804:
        return 0x30f040a + reg_6804_width + (reg_6804_height << 16);
    case 0xC0F06824:
        return 0x4ca;
    case 0xC0F06828:
        return 0x4ca;
...
static inline uint32_t reg_override_5K_3x1_EOSM(uint32_t reg, uint32_t old_val)
{
    ...
    if (ratios == 0x1)
    {
        ...
        {
        case 0xC0F06804:
            return 0x2e30504 + reg_6804_width + (reg_6804_height << 16);
        case 0xC0F06824:
            return 0x56a;
        case 0xC0F06828:
            return 0x56a;
        ...
        case 0xC0F0600C:
            return 0x57b057b + reg_6008 + (reg_6008 << 16);
        case 0xC0F06014:
            return 0x4b5 + reg_6014;
        case 0xC0F0713c:
            return 0x2e0 + reg_713c;
        case 0xC0F07150:
            return 0x299 + reg_7150;
    ...
    else
    {
        ...
        {
        case 0xC0F06804:
            return 0x2e50506 + reg_6804_width + (reg_6804_height << 16);
        case 0xC0F06824:
            return 0x56a;
        case 0xC0F06828:
            return 0x56a;
        ...
        case 0xC0F06014:
            return 0x3b5 + reg_6014;
        case 0xC0F0713c:
            return 0x2e4 + reg_713c;
        case 0xC0F07150:
            return 0x2ef + reg_7150;
...
static inline uint32_t reg_override_4K_5x1_EOSM(uint32_t reg, uint32_t old_val)
{
    ...
    {
    case 0xC0F06804:
        return 0x2d7040a + reg_6804_width + (reg_6804_height << 16);
    case 0xC0F06824:
        return 0x4ca;
    case 0xC0F06828:
        return 0x4ca;
    ...
    case 0xC0F06830:
        return 0x4ca;
    case 0xC0F06010:
        return 0x50f + reg_6008;
    case 0xC0F06008:
        return 0x50f050f + reg_6008 + (reg_6008 << 16);
    case 0xC0F0600C:
        return 0x50f050f + reg_6008 + (reg_6008 << 16);
    case 0xC0F06014:
        return 0x405 + reg_6014;
...
static inline uint32_t reg_override_3x3_mv1080_eosm(uint32_t reg, uint32_t old_val)
{
    ...
    if ((ratios == 0x1) || (ratios == 0x2) || (ratios == 0x3))
    {
        ...
        {
        case 0xC0F06014:
            return set_25fps == 0x1 ? 0xa03 - 103 + reg_6014 : 0xa03 + reg_6014;
        case 0xC0F0600c:
            return 0x2070207 + reg_6008 + (reg_6008 << 16);
        case 0xC0F06008:
            return 0x2070207 + reg_6008 + (reg_6008 << 16);
        case 0xC0F06010:
            return 0x207 + reg_6008;
        }
    }
    ...
    {
    case 0xC0F06804:
        return 0x4a701d4 + reg_6804_width + (reg_6804_height << 16);
    case 0xC0F0713c:
        return 0x4a7 + reg_713c;
    case 0xC0F07150:
        return 0x4a0 + reg_7150;
...
static inline uint32_t reg_override_mcm_mv1080_eosm(uint32_t reg, uint32_t old_val)
{
    ...
    if ((ratios == 0x0 && x3crop == 0x0) || (ratios == 0x0 && x3crop == 0x1))
    {
        ...
        {
        case 0xC0F06804:
            return 0x4a601e4 + reg_6804_width + (reg_6804_height << 16);
        case 0xC0F0713c:
            return 0x4a7 + reg_713c;
        case 0xC0F07150:
            return 0x430 + reg_7150;
        }
    }
    ...
    if ((ratios == 0x1 || ratios == 0x2 || ratios == 0x3) && x3crop == 0x0)
    {
        ...
        {
        case 0xC0F06804:
            return 0x42401e4 + reg_6804_width + (reg_6804_height << 16);
        case 0xC0F0713c:
            return 0x424;
        case 0xC0F07150:
            return 0x38f;
    ...
    if ((ratios == 0x1 || ratios == 0x2) && x3crop == 0x1)
    {
        ...
        {
        case 0xC0F06804:
            return 0x45601e4 + reg_6804_width + (reg_6804_height << 16);
        case 0xC0F0713c:
            return 0x457 + reg_713c;
        case 0xC0F07150:
            return 0x3e0 + reg_7150;
        }
    }
    ...
    if (set_25fps == 0x1)
    {
        ...
        {
        case 0xC0F06014:
            return set_25fps == 0x1 ? 0x98c - 101 + reg_6014 : 0x98c + reg_6014;
        case 0xC0F0600c:
            return 0x2210221 + reg_6008 + (reg_6008 << 16);
        case 0xC0F06008:
            return 0x2210221 + reg_6008 + (reg_6008 << 16);
        case 0xC0F06010:
            return 0x221 + reg_6008;

        case 0xC0F06824:
            return 0x21f + reg_6824;
        case 0xC0F06828:
            return 0x21f + reg_6824;
        case 0xC0F0682C:
            return 0x21f + reg_6824;
    ...
    case 0xc0f0b134:
        return (ratios == 0x1 || ratios == 0x2) ? 0x5 : 0x4;
    }
...
static inline uint32_t reg_override_3x3_48fps_eosm(uint32_t reg, uint32_t old_val)
{
    ...
    if ((ratios != 0x1) && (ratios != 0x2) && (ratios != 0x3))
    {
        ...
        {
        case 0xC0F06804:
            return 0x3ef01d4 + reg_6804_width + (reg_6804_height << 16);
        case 0xC0F0713c:
            return 0x3e1 + reg_713c;
        case 0xC0F07150:
            return 0x3dc + reg_7150;

        /* 46 fps */
        case 0xC0F06014:
            return 0x525 + reg_6014;
    ...
    if (ratios == 0x1 || ratios == 0x2)
    {
        ...
        {
        case 0xC0F06804:
            return 0x2f701d4 + reg_6804_width + (reg_6804_height << 16);
        case 0xC0F0713c:
            return 0x33d + reg_713c;
        case 0xC0F07150:
            return 0x2fa + reg_7150;

        /* 48/50fps. 50 if set_25fps is on */
        case 0xC0F06014:
            return set_25fps == 0x1 ? 0x4a1 + reg_6014 : 0x4a2 + reg_6014;
        case 0xC0F0600c:
            return set_25fps == 0x1 ? 0x21b021b + reg_6008 + (reg_6008 << 16) : 0x2310231 + reg_6008 + (reg_6008 << 16);
        case 0xC0F06008:
            return set_25fps == 0x1 ? 0x21b021b + reg_6008 + (reg_6008 << 16) : 0x2310231 + reg_6008 + (reg_6008 << 16);
        case 0xC0F06010:
            return set_25fps == 0x1 ? 0x21b + reg_6008 : 0x231 + reg_6008;
    ...
    if (ratios == 0x3)
    {
        ...
        case 0xC0F06804:
            return 0x3ef01d4 + reg_6804_width + (reg_6804_height << 16);
        case 0xC0F0713c:
            return 0x3e3 + reg_713c;
        case 0xC0F07150:
            return 0x3dd + reg_7150;

        /* 46 fps */
        case 0xC0F06014:
            return 0x539 + reg_6014;

        case 0xC0F0600c:
            return 0x2130213 + reg_6008 + (reg_6008 << 16);
        case 0xC0F06008:
            return 0x2130213 + reg_6008 + (reg_6008 << 16);
        case 0xC0F06010:
            return 0x213 + reg_6008;
...
static inline uint32_t reg_override_3x1_mv720_50fps_eosm(uint32_t reg, uint32_t old_val)
{
    ...
    {
    case 0xC0F06804:
        return 0x2d701d4 + reg_6804_width + (reg_6804_height << 16);
    case 0xC0F0713c:
        return 0x2d7 + reg_713c;
    case 0xC0F07150:
        return 0x2d0 + reg_7150;

    /* 50 fps */
    case 0xC0F06014:
        return 0x4aa + reg_6014;
    case 0xC0F0600c:
        return 0x2170217 + reg_6008 + (reg_6008 << 16);
    case 0xC0F06008:
        return 0x2170217 + reg_6008 + (reg_6008 << 16);
    case 0xC0F06010:
        return 0x217 + reg_6008;
...
static inline uint32_t reg_override_anamorphic_rewired_eosm(uint32_t reg, uint32_t old_val)
{
    ...
    if (ratios == 0x1 || ratios == 0x2)
    {
        ...
        case 0xC0F06824:
            return bitdepth == 0x4 ? 0x6d4 + reg_6824 : 0x79d + reg_6824;
        case 0xC0F06828:
            return bitdepth == 0x4 ? 0x6d4 + reg_6824 : 0x79d + reg_6824;
        case 0xC0F0682C:
            return bitdepth == 0x4 ? 0x6d4 + reg_6824 : 0x79d + reg_6824;
    ...
    else
    {
        ...
        case 0xC0F06824:
            return bitdepth == 0x4 ? 0x738 + reg_6824 : 0x79d + reg_6824;
        case 0xC0F06828:
            return bitdepth == 0x4 ? 0x738 + reg_6824 : 0x79d + reg_6824;
        case 0xC0F0682C:
            return bitdepth == 0x4 ? 0x738 + reg_6824 : 0x79d + reg_6824;
...
static inline uint32_t reg_override_anamorphic_eosm(uint32_t reg, uint32_t old_val)
{
    ...
    if (ratios == 0x1 || ratios == 0x2)
    {
        ...
        {
        case 0xC0F06804:
            return 0x79f01d4 + reg_6804_width + (reg_6804_height << 16);

        case 0xC0F06014:
            return 0x8ec + reg_6014;
        case 0xC0F0600c:
            return set_25fps == 0x1 ? 0x2470247 - 24 + reg_6008 + (reg_6008 << 16) : 0x2470247 + reg_6008 + (reg_6008 << 16);
        case 0xC0F06008:
            return set_25fps == 0x1 ? 0x2470247 - 24 + reg_6008 + (reg_6008 << 16) : 0x2470247 + reg_6008 + (reg_6008 << 16);
        case 0xC0F06010:
            return set_25fps == 0x1 ? 0x247 - 24 + reg_6008 : 0x247 + reg_6008;

        case 0xC0F0713c:
            return 0x797 + reg_713c;
        case 0xC0F07150:
            return 0x791 + reg_7150;
    ...
    if (ratios == 0x3)
    {
        ...
        {
        case 0xC0F06804:
            return 0x7ef01d4 + reg_6804_width + (reg_6804_height << 16);

        case 0xC0F06014:
            return 0x95d + reg_6014;
        case 0xC0F0600c:
            return set_25fps == 0x1 ? 0x22b022b - 22 + reg_6008 + (reg_6008 << 16) : 0x22b022b + reg_6008 + (reg_6008 << 16);
        case 0xC0F06008:
            return set_25fps == 0x1 ? 0x22b022b - 22 + reg_6008 + (reg_6008 << 16) : 0x22b022b + reg_6008 + (reg_6008 << 16);
        case 0xC0F06010:
            return set_25fps == 0x1 ? 0x22b - 24 + reg_6008 : 0x22b + reg_6008;

        case 0xC0F0713c:
            return 0x7ef + reg_713c;
        case 0xC0F07150:
            return 0x7dd + reg_7150;
    ...
    if ((ratios != 0x1) && (ratios != 0x2) && (ratios != 0x3))
    {
        ...
        {
        case 0xC0F06804:
            return 0x88501c2 + reg_6804_width + (reg_6804_height << 16);

        case 0xC0F06014:
            return 0x99d + reg_6014;
        case 0xC0F0600c:
            return 0x21d021d + reg_6008 + (reg_6008 << 16);
        case 0xC0F06008:
            return 0x21d021d + reg_6008 + (reg_6008 << 16);
        case 0xC0F06010:
            return 0x21d + reg_6008;

        case 0xC0F0713c:
            return 0x885 + reg_713c;
        case 0xC0F07150:
            return 0x880 + reg_7150;
...
static inline uint32_t reg_override_2520_700d(uint32_t reg, uint32_t old_val)
{
    ...
    /* X: (3072+140)/8 + 0x17, adjusted for 3072 in raw_rec */
    case 0xC0F06804:
        return 0x5840298 + reg_6804_width + (reg_6804_height << 16); // 2520x1386  x5 Mode;

    case 0xC0F06014:
        return 0x747 + reg_6014;

    case 0xC0F0713c:
        return 0x5C7 + reg_713c;
    case 0xC0F07150:
        return 0x59A + reg_7150;
    }
...
static inline uint32_t reg_override_3K_700d(uint32_t reg, uint32_t old_val)
{
    ...
    /* X: (3072+140)/8 + 0x17, adjusted for 3072 in raw_rec */
    case 0xC0F06804:
        return 0x5340322 + reg_6804_width + (reg_6804_height << 16); // 3072x1306  x5 Mode;

    case 0xC0F06824:
        return 0x3ca + reg_6824;
    case 0xC0F06828:
        return 0x3ca + reg_6824;
    case 0xC0F0682C:
        return 0x3ca + reg_6824;
    ...
    case 0xC0F06010:
        return 0x37b + reg_6008 + (reg_6008 << 16);
    case 0xC0F06008:
        return 0x37b037b + reg_6008 + (reg_6008 << 16);
    case 0xC0F0600C:
        return 0x37b037b + reg_6008 + (reg_6008 << 16);

    case 0xC0F06014:
        return 0x6d7 + reg_6014;

    case 0xC0F0713c:
        return 0x555 + reg_713c;
    }
...
static inline uint32_t reg_override_3540(uint32_t reg, uint32_t old_val)
{
    ...
    /* X: (3072+140)/8 + 0x17, adjusted for 3072 in raw_rec */
    case 0xC0F06804:
        return 0x4540398 + reg_6804_width + (reg_6804_height << 16); // 3544x1080  x5 Mode;
    ...
    case 0xC0F06010:
        return 0x3cb + reg_6008 + (reg_6008 << 16);
    case 0xC0F06008:
        return 0x3cb03cb + reg_6008 + (reg_6008 << 16);
    case 0xC0F0600C:
        return 0x3cb03cb + reg_6008 + (reg_6008 << 16);
    }
...
static inline uint32_t reg_override_UHD_700d(uint32_t reg, uint32_t old_val)
{
    ...
    /* X: (3072+140)/8 + 0x17, adjusted for 3072 in raw_rec */
    case 0xC0F06804:
        return 0x88d03e2 + reg_6804_width + (reg_6804_height << 16); // 3840x2160  x5 Mode;

    case 0xC0F06824:
        return 0x4ca + reg_6824;
    case 0xC0F06828:
        return 0x4ca + reg_6824;
    case 0xC0F0682C:
        return 0x4ca + reg_6824;
    ...
    case 0xC0F06010:
        return 0x43b + reg_6008 + (reg_6008 << 16);
    case 0xC0F06008:
        return 0x43b043b + reg_6008 + (reg_6008 << 16);
    case 0xC0F0600C:
        return 0x43b043b + reg_6008 + (reg_6008 << 16);

    case 0xC0F06014:
        return 0x993 + reg_6014;

    case 0xC0F0713c:
        return 0x8E7 + reg_713c;
    case 0xC0F07150:
        return 0x8BA + reg_7150;
    }
...
static inline uint32_t reg_override_4K_700d(uint32_t reg, uint32_t old_val)
{
    ...
    /* X: (3072+140)/8 + 0x17, adjusted for 3072 in raw_rec */
    case 0xC0F06804:
        return 0xA1C0422 + reg_6804_width + (reg_6804_height << 16); // 4096x2560  x5 Mode;
    ...
    case 0xC0F06010:
        return 0x45b + reg_6008 + (reg_6008 << 16);
    case 0xC0F06008:
        return 0x45b045b + reg_6008 + (reg_6008 << 16);
    case 0xC0F0600C:
        return 0x45b045b + reg_6008 + (reg_6008 << 16);
    ...
    case 0xC0F06014:
        return 0xBD3 + reg_6014;

    case 0xC0F0713c:
        return 0xA55 + reg_713c;
    }
...
static inline uint32_t reg_override_fullres_lv_700d(uint32_t reg, uint32_t old_val)
{
    ...
    /* X: (3072+140)/8 + 0x17, adjusted for 3072 in raw_rec */
    case 0xC0F06804:
        return 0xcbc0538 + reg_6804_width + (reg_6804_height << 16); // 2520x1386  x5 Mode;
    case 0xC0F06800:
        return 0x30010;

    case 0xC0F06824:
        return 0x56a + reg_6824;
    case 0xC0F06828:
        return 0x56a + reg_6824;
    case 0xC0F0682C:
        return 0x56a + reg_6824;
    ...
    case 0xC0F06010:
        return 0x56b + reg_6008 + (reg_6008 << 16);
    case 0xC0F06008:
        return 0x56b056b + reg_6008 + (reg_6008 << 16);
    case 0xC0F0600C:
        return 0x56b056b + reg_6008 + (reg_6008 << 16);

    case 0xC0F06014:
        return 0xdcb + reg_6014;

    case 0xC0F0713c:
        return 0xca7 + reg_713c;
    }
...
static inline uint32_t reg_override_3x3_mv1080_700d(uint32_t reg, uint32_t old_val)
{
    ...
    /* X: (3072+140)/8 + 0x17, adjusted for 3072 in raw_rec */
    case 0xC0F06804:
        return 0x4a601d4 + reg_6804_width + (reg_6804_height << 16);

    case 0xC0F06014:
        return 0x7cf + reg_6014;
    case 0xC0F0600c:
        return 0x27f027f + reg_6008 + (reg_6008 << 16);
    case 0xC0F06008:
        return 0x27f027f + reg_6008 + (reg_6008 << 16);
    case 0xC0F06010:
        return 0x27f + reg_6008 + (reg_6008 << 16);
...
static inline uint32_t reg_override_HD100_6D(uint32_t reg, uint32_t old_val)
{
    ...
    case 0xC0F0713c:
        return 0x1a7; // Same as 6804?; */
    case 0xC0F07150:
        return 0x151; // change with vertical res change - 6804->664 by default value for 7150 = 268 */
...
static inline uint32_t reg_override_HD75_6D(uint32_t reg, uint32_t old_val)
{
    ...
    case 0xC0F0713c:
        return 0x24e; // Same as 6804?; */
    case 0xC0F07150:
        return 0x151; // change with vertical res change - 6804->664 by default value for
    }
...
static inline uint32_t reg_override_HD50_6D(uint32_t reg, uint32_t old_val)
{
    ...
    /* raw resolution (end line/column) */
    case 0xC0F06804:
        return 0x35101ea; // 1832 x 869 6d; */
...
static inline uint32_t reg_override_1x3_6D(uint32_t reg, uint32_t old_val)
{
    ...
    {
    case 0xC0F06804:
        return 0x8230150; // 1640 x 2052 25 fps
...
static inline uint32_t reg_override_1x3_24FPS_6D(uint32_t reg, uint32_t old_val)
{
    ...
    /* raw resolution (end line/column) */
    case 0xC0F06804:
        return 0x87f01ba; // 1640 x 2144 24 fps
...
static inline uint32_t reg_override_2K_6D(uint32_t reg, uint32_t old_val)
{
    ...
    /* X: (3072+140)/8 + 0x17, adjusted for 3072 in raw_rec */
    case 0xC0F06804:
        return 0x52f02c0; // 2688x1296_25fps x5 Mode;
...
static inline uint32_t reg_override_ULTRA_HD_6D(uint32_t reg, uint32_t old_val)
{
    ...
    case 0xC0F06804:
        return 0x4ce02f0; // UHD for 6d   was 3dd02c0
...
static inline uint32_t reg_override_5K_6D(uint32_t reg, uint32_t old_val)
{
    ...
    /* raw resolution (end line/column) */
    case 0xC0F06804:
        return 0x2800520; // 5472x3604 5x zoom;
...
static inline uint32_t reg_override_FullRes_6D(uint32_t reg, uint32_t old_val)
{
    ...
    /* raw resolution (end line/column) */
    case 0xC0F06804:
        return 0xc1e0576; // 5472x3078 5x zoom;  c24
    ...
    case 0xC0F06014:
        return 0x1c16; // for 2fps use 0x231c for 5.6 fps 0xc7f;
...
static inline uint32_t reg_override_HD2_6D(uint32_t reg, uint32_t old_val)
{
    ...
    {
    case 0xC0F06804:
        return 0x593028c; // 2480x1396_25fps x5 Mode;
...
static inline uint32_t reg_override_zoom_fps(uint32_t reg, uint32_t old_val)
{
    ...
    int timerA =
        (video_mode_fps == 24) ? 512 : (video_mode_fps == 25) ? 512 : (video_mode_fps == 30) ? 520 : (video_mode_fps == 50) ? 512 :     /* cannot get 50, use 25 */
                                                                                                         (video_mode_fps == 60) ? 520 : /* cannot get 60, use 30 */
                                                                                                             -1;
    int timerB =
        (video_mode_fps == 24) ? 1955 : (video_mode_fps == 25) ? 1875 : (video_mode_fps == 30) ? 1540 : (video_mode_fps == 50) ? 1875 : (video_mode_fps == 60) ? 1540 : -1;
...
static void *get_engio_reg_override_func()
{
    uint32_t (*reg_override_func)(uint32_t, uint32_t) =
        (crop_preset == CROP_PRESET_3X) ? reg_override_mv1080_mv720p :                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      /* fixme: corrupted image */
            (crop_preset == CROP_PRESET_3X_TALL) ? reg_override_3X_tall : (crop_preset == CROP_PRESET_3x3_1X_50p) ? reg_override_3x3_50p : (crop_preset == CROP_PRESET_3x3_1X_60p) ? reg_override_3x3_60p : (crop_preset == CROP_PRESET_3x3_1X_45p) ? reg_override_3x3_45p : (crop_preset == CROP_PRESET_3x3_1X_48p) ? reg_override_3x3_48p : (crop_preset == CROP_PRESET_3K) ? reg_override_3K : (crop_preset == CROP_PRESET_4K_HFPS) ? reg_override_4K_hfps : (crop_preset == CROP_PRESET_UHD) ? reg_override_UHD : (crop_preset == CROP_PRESET_40_FPS) ? reg_override_40_fps : (crop_preset == CROP_PRESET_FULLRES_LV) ? reg_override_fullres_lv : (crop_preset == CROP_PRESET_CENTER_Z) ? reg_override_zoom_fps : (crop_preset == CROP_PRESET_1x3) ? reg_override_1x3 : (crop_preset == CROP_PRESET_1x3_17fps) ? reg_override_1x3_17fps : (crop_preset == CROP_PRESET_mv1080_mv720p) ? reg_override_mv1080_mv720p : (crop_preset == CROP_PRESET_mv1080p_mv720p_100D) ? reg_override_3xcropmode_100d : (crop_preset == CROP_PRESET_3xcropmode_100D) ? reg_override_3xcropmode_100d : (crop_preset == CROP_PRESET_2K_100D) ? reg_override_2K_100d : (crop_preset == CROP_PRESET_3K_100D) ? reg_override_3K_100d : (crop_preset == CROP_PRESET_4K_100D) ? reg_override_4K_100d : (crop_preset == CROP_PRESET_4K_3x1_100D) ? reg_override_4K_3x1_100D : (crop_preset == CROP_PRESET_5K_3x1_100D) ? reg_override_5K_3x1_100D : (crop_preset == CROP_PRESET_1080K_100D) ? reg_override_1080p_100d : (crop_preset == CROP_PRESET_anamorphic_rewired_100D) ? reg_override_anamorphic_rewired_100d : (crop_preset == CROP_PRESET_CENTER_Z_EOSM) ? reg_override_center_z_eosm : (crop_preset == CROP_PRESET_2K_EOSM) ? reg_override_2K_eosm : (crop_preset == CROP_PRESET_3K_EOSM) ? reg_override_3K_eosm : (crop_preset == CROP_PRESET_4K_EOSM) ? reg_override_4K_eosm : (crop_preset == CROP_PRESET_4K_3x1_EOSM) ? reg_override_4K_3x1_EOSM : (crop_preset == CROP_PRESET_5K_3x1_EOSM) ? reg_override_5K_3x1_EOSM : (crop_preset == CROP_PRESET_4K_5x1_EOSM) ? reg_override_4K_5x1_EOSM : (crop_preset == CROP_PRESET_3x3_mv1080_EOSM) ? reg_override_3x3_mv1080_eosm : (crop_preset == CROP_PRESET_mcm_mv1080_EOSM) ? reg_override_mcm_mv1080_eosm : (crop_preset == CROP_PRESET_3x3_mv1080_48fps_EOSM) ? reg_override_3x3_48fps_eosm : (crop_preset == CROP_PRESET_3x3_mv1080_48fps_700D) ? reg_override_3x3_48fps_eosm :     //Same as for eosm?
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         (crop_preset == CROP_PRESET_3x3_mv1080_48fps_650D) ? reg_override_3x3_48fps_eosm : //Same as for eosm?
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             (crop_preset == CROP_PRESET_3x1_mv720_50fps_EOSM) ? reg_override_3x1_mv720_50fps_eosm : (crop_preset == CROP_PRESET_anamorphic_rewired_EOSM) ? reg_override_anamorphic_rewired_eosm : (crop_preset == CROP_PRESET_anamorphic_EOSM) ? reg_override_anamorphic_eosm : (crop_preset == CROP_PRESET_3x3_1X_EOSM) ? reg_override_mv1080_mv720p : (crop_preset == CROP_PRESET_3x3_1X_100D) ? reg_override_mv1080_mv720p : (crop_preset == CROP_PRESET_x10_EOSM) ? reg_override_x10_eosm : (crop_preset == CROP_PRESET_OFF_eosm) ? reg_override_crop_preset_off_eosm : (crop_preset == CROP_PRESET_2520_1418_700D) ? reg_override_2520_700d : (crop_preset == CROP_PRESET_2520_1418_650D) ? reg_override_2520_700d : (crop_preset == CROP_PRESET_3K_700D) ? reg_override_3K_700d : (crop_preset == CROP_PRESET_3K_650D) ? reg_override_3K_700d : (crop_preset == CROP_PRESET_3540_700D) ? reg_override_3540 : (crop_preset == CROP_PRESET_3540_650D) ? reg_override_3540 : (crop_preset == CROP_PRESET_4K_700D) ? reg_override_4K_700d : (crop_preset == CROP_PRESET_4K_650D) ? reg_override_4K_700d : (crop_preset == CROP_PRESET_UHD_700D) ? reg_override_UHD_700d : (crop_preset == CROP_PRESET_UHD_650D) ? reg_override_UHD_700d : (crop_preset == CROP_PRESET_FULLRES_LV_700D) ? reg_override_fullres_lv_700d : (crop_preset == CROP_PRESET_FULLRES_LV_650D) ? reg_override_fullres_lv_700d : (crop_preset == CROP_PRESET_3x3_mv1080_700D) ? reg_override_3x3_mv1080_700d : (crop_preset == CROP_PRESET_3x3_mv1080_650D) ? reg_override_3x3_mv1080_700d : (crop_preset == CROP_PRESET_anamorphic_700D) ? reg_override_anamorphic_700d : (crop_preset == CROP_PRESET_anamorphic_650D) ? reg_override_anamorphic_700d : (crop_preset == CROP_PRESET_CENTER_Z_700D) ? reg_override_center_z_700d : (crop_preset == CROP_PRESET_CENTER_Z_650D) ? reg_override_center_z_700d : (crop_preset == CROP_PRESET_HD100_6D) ? reg_override_HD100_6D : (crop_preset == CROP_PRESET_HD75_6D) ? reg_override_HD75_6D : (crop_preset == CROP_PRESET_HD50_6D) ? reg_override_HD50_6D : (crop_preset == CROP_PRESET_1x3_6D) ? reg_override_1x3_6D : (crop_preset == CROP_PRESET_1x3_24FPS_6D) ? reg_override_1x3_24FPS_6D : (crop_preset == CROP_PRESET_2K_6D) ? reg_override_2K_6D : (crop_preset == CROP_PRESET_ULTRA_HD_6D) ? reg_override_ULTRA_HD_6D : (crop_preset == CROP_PRESET_4K_6D) ? reg_override_4K_6D : (crop_preset == CROP_PRESET_5K_6D) ? reg_override_5K_6D : (crop_preset == CROP_PRESET_FullRes_6D) ? reg_override_FullRes_6D : (crop_preset == CROP_PRESET_HD2_6D) ? reg_override_HD2_6D : 0;
...
static MENU_UPDATE_FUNC(crop_update)
{
    ...
    if ((CROP_PRESET_MENU && lv) && (is_EOSM))
    {
        ...
        if ((lv_dispsize == 1) &&
            ((CROP_PRESET_MENU == CROP_PRESET_4K_3x1_EOSM) || (CROP_PRESET_MENU == CROP_PRESET_5K_3x1_EOSM) || (CROP_PRESET_MENU == CROP_PRESET_2K_EOSM) || (CROP_PRESET_MENU == CROP_PRESET_3K_EOSM) || (CROP_PRESET_MENU == CROP_PRESET_4K_EOSM) || (CROP_PRESET_MENU == CROP_PRESET_5K_3x1_EOSM)))
        {
        ...
        if ((lv_dispsize > 1) &&
            ((CROP_PRESET_MENU == CROP_PRESET_3x3_mv1080_EOSM) || (CROP_PRESET_MENU == CROP_PRESET_mcm_mv1080_EOSM) || (CROP_PRESET_MENU == CROP_PRESET_3x3_mv1080_48fps_EOSM) || (CROP_PRESET_MENU == CROP_PRESET_3x1_mv720_50fps_EOSM) || (CROP_PRESET_MENU == CROP_PRESET_anamorphic_rewired_EOSM) || (CROP_PRESET_MENU == CROP_PRESET_anamorphic_EOSM)))
        {
    ...
    if ((CROP_PRESET_MENU && lv) && (is_100D))
    {
        ...
        if ((lv_dispsize == 1) &&
            ((CROP_PRESET_MENU == CROP_PRESET_4K_3x1_100D) || (CROP_PRESET_MENU == CROP_PRESET_5K_3x1_100D) || (CROP_PRESET_MENU == CROP_PRESET_2K_100D) || (CROP_PRESET_MENU == CROP_PRESET_3K_100D) || (CROP_PRESET_MENU == CROP_PRESET_4K_100D) || (CROP_PRESET_MENU == CROP_PRESET_1080K_100D)))
        {
            MENU_SET_WARNING(MENU_WARN_NOT_WORKING, "This preset only works in x5 zoom");
            return;
        }
    ...
    if ((lv_dispsize > 1) &&
        ((CROP_PRESET_MENU == CROP_PRESET_3x3_1X_100D) || (CROP_PRESET_MENU == CROP_PRESET_mv1080p_mv720p_100D) || (CROP_PRESET_MENU == CROP_PRESET_anamorphic_rewired_100D) || (CROP_PRESET_MENU == CROP_PRESET_3xcropmode_100D)))
    {
        MENU_SET_WARNING(MENU_WARN_NOT_WORKING, "This preset only works in x1 movie mode");
        return;
    }
...
static struct menu_entry presets_switch_menu[] =
    {
        ...
        {.depends_on = DEP_MOVIE_MODE, .name = "mv1080p MCM rewire x3crop", .select = apply_preset_mv1080_x3crop, .priv = "ps_entry", .help = "HD. Crop factor>4.66. 14bit. Less aliasing. Continuous recording possible.", .help2 = "Realtime preview. Shoot continuously with SanDisk Extreme Pro 95MB/s.", .children = (struct menu_entry[]){
                                                                                                                                                                                                                                                                                                                  {.name = "ratios", .priv = &ratios_preset_mv1080_x3crop, .max = 3, .choices = ratios_choices, .help = "Change aspect ratio\n"},
                                                                                                                                                                                                                                                                                                                  MENU_EOL,
                                                                                                                                                                                                                                                                                                              }},
        {.depends_on = DEP_MOVIE_MODE, .name = "5K Anamorphic", .select = apply_preset_5K_anamorphic, .priv = "ps_entry", .help = "5K. Crop factor>1.68. 10bit. Less aliasing. Continuous recording possible.", .help2 = "Slow preview. Desqueeze in post.", .children = (struct menu_entry[]){
                                                                                                                                                                                                                                                                 {.name = "ratios", .priv = &ratios_preset_5K_anamorphic, .max = 3, .choices = ratios_choices, .help = "Change aspect ratio\n"},
                                                                                                                                                                                                                                                                 MENU_EOL,
                                                                                                                                                                                                                                                             }},
        {.depends_on = DEP_MOVIE_MODE, .name = "2.5K", .select = apply_preset_2K, .priv = "ps_entry", .help = "2K. Crop factor>3.83. 10bit. Less aliasing. Continuous recording possible.", .help2 = "Slow preview. Shoot continuously with SanDisk Extreme Pro 95MB/s.", .children = (struct menu_entry[]){
                                                                                                                                                                                                                                                                              {.name = "ratios", .priv = &ratios_preset_2K, .max = 3, .choices = ratios_choices, .help = "Change aspect ratio\n"},
                                                                                                                                                                                                                                                                              MENU_EOL,
                                                                                                                                                                                                                                                                          }},
        {.depends_on = DEP_MOVIE_MODE, .name = "mv1080p 45/46/48/50fps", .select = apply_preset_mv1080_high_framerate, .priv = "ps_entry", .help = "HD. Crop factor>1.61. 10bit. Aliasing. Continuous recording possible.", .help2 = "Realtime preview. 16:9=45fps. 2.35:1&2.39:1=48fps or 50fps if set to 25fps.", .children = (struct menu_entry[]){
                                                                                                                                                                                                                                                                                                                        {.name = "ratios", .priv = &ratios_preset_mv1080_high_framerate, .max = 3, .choices = ratios_choices, .help = "Change aspect ratio\n"},
                                                                                                                                                                                                                                                                                                                        MENU_EOL,
                                                                                                                                                                                                                                                                                                                    }},
        {.depends_on = DEP_MOVIE_MODE, .name = "mv1080p 45/46/48/50fps x3crop", .select = apply_preset_mv1080_high_framerate_x3crop, .priv = "ps_entry", .help = "HD. Crop factor>4.66. 10bit. Less aliasing. Continuous recording possible.", .help2 = "Realtime preview. 16:9=45fps. 2.35:1&2.39:1=48fps or 50fps if set to 25fps.", .children = (struct menu_entry[]){
                                                                                                                                                                                                                                                                                                                                           {.name = "ratios", .priv = &ratios_preset_mv1080_high_framerate_x3crop, .max = 3, .choices = ratios_choices, .help = "Change aspect ratio\n"},
                                                                                                                                                                                                                                                                                                                                           MENU_EOL,
                                                                                                                                                                                                                                                                                                                                       }},
        {
            .depends_on = DEP_MOVIE_MODE,
        ...
        {.depends_on = DEP_MOVIE_MODE, .name = "Toggle mv1080", .priv = &x3toggle, .max = 2, .choices = CHOICES("OFF", "press down", "SET"), .help = "Press key 1x: Jumps into mv1080p rewire from another preset, or...", .help2 = "...toggle x3crop. Press key 2x: Toggle high framerate mode."},
        MENU_EOL,
};
...
static MENU_UPDATE_FUNC(switch_menu_toggle_update)
{
    // Hide this entry on unsupported cameras
    // Don't use hidden, it will show a warning that it can be re-enabled (which it can't)
    // TODO: shidden is annoying because the item disappears the first time the menu is opened
    // and isn't remembered between reboots
    // Find better way: don't make this item in the first place, or remove it before menu is opened
    entry->shidden = !is_EOSM;
    MENU_SET_VALUE(switch_menu_enabled ? "ON" : "OFF");
}
...
static int crop_rec_needs_lv_refresh()
{
    ...
    if (is_EOSM && (shamem_read(0xc0f383d4) == 0x4f0010 && (CROP_PRESET_MENU != CROP_PRESET_mcm_mv1080_EOSM)) &&
 ...
 * (H.264 should tolerate this pretty well, except maybe 50D) */
    ...
    if (!crop_patch2 && get_halfshutter_pressed() && (zoomaid == 0x1 || zoomaid == 0x2) && !is_6D && !is_5D3)
    {
        ...
        if (get_halfshutter_pressed() && (zoomaid == 0x1 || zoomaid == 0x2))
        {
                ...
                CROP_PRESET_MENU != CROP_PRESET_anamorphic_rewired_100D)
            {
                ...
                if ((zoomaid == 0x1 || zoomaid == 0x2))
                    set_lv_zoom(10);
            }
            else
            {
                if (crop_preset == CROP_PRESET_x10_EOSM)
                    movie_crop_hack_disable();
    ...
    if (x3toggle != 0x1 || x3toggle != 0x2 || zoomaid != 0x0)
    {
        crop_patch = 0; //disable patch while off
    }
...
static LVINFO_UPDATE_FUNC(crop_info)
{
    ...
    if (CROP_PRESET_MENU == CROP_PRESET_4K_100D)
    {
        ...
        if (ratios == 0x1 && timelapse == 0x0)
            snprintf(buffer, sizeof(buffer), "2.39:1");
        if (ratios == 0x2 && timelapse == 0x0)
            snprintf(buffer, sizeof(buffer), "2.35:1");
        if (ratios == 0x3 && timelapse == 0x0)
            snprintf(buffer, sizeof(buffer), "16:9");
        if (timelapse == 0x1 || timelapse == 0x7)
            snprintf(buffer, sizeof(buffer), "timelapse0.4fps");
        if (timelapse == 0x2 || timelapse == 0x8)
            snprintf(buffer, sizeof(buffer), "timelapse 1fps");
        if (timelapse == 0x3 || timelapse == 0x9)
            snprintf(buffer, sizeof(buffer), "timelapse 2fps");
        if (timelapse == 0x4)
            snprintf(buffer, sizeof(buffer), "timelapse 3fps");
        if (timelapse == 0x5)
            snprintf(buffer, sizeof(buffer), "timelapse 4fps");
        if (timelapse == 0x6)
            snprintf(buffer, sizeof(buffer), "timelapse 5fps");
    }
    ...
    {
        if (ratios == 0x0)
            snprintf(buffer, sizeof(buffer), "mv1080p_46fps");
        if (ratios == 0x1 || ratios == 0x2)
            snprintf(buffer, sizeof(buffer), "mv1080p_48fps");
        if (ratios == 0x3)
            snprintf(buffer, sizeof(buffer), "mv1080p_45fps");
        if ((ratios == 0x1 || ratios == 0x2) && set_25fps == 0x1)
...
static unsigned int raw_info_update_cbr(unsigned int unused)
{
    if (patch_active)
    {
        ...
        case CROP_PRESET_anamorphic_650D:
        case CROP_PRESET_anamorphic_rewired_100D:
            raw_capture_info.binning_x = 3;
            raw_capture_info.skipping_x = 0;
            break;
        ...
        case CROP_PRESET_5K_3x1_100D:
            if (lv_dispsize == 1)
            {
                raw_capture_info.binning_x = 3;
                raw_capture_info.skipping_x = 0;
                raw_capture_info.binning_y = 1;
                raw_capture_info.skipping_y = 0;
            }
            else
            {
                raw_capture_info.binning_x = 1;
                raw_capture_info.skipping_x = 0;
                raw_capture_info.binning_y = 3;
                raw_capture_info.skipping_y = 0;
            }
            break;
