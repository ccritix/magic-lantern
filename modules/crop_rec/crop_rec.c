...
static inline uint32_t reg_override_bits(uint32_t reg, uint32_t old_val)
{
    ...
    if (!RECORDING && (is_EOSM || is_100D || is_5D3))
    {
        ...
        /* reset eosm switch */
        if (bitdepth == 0x0 && (is_EOSM || is_100D || is_5D3))
        {
        ...
        .children =  (struct menu_entry[]) {
            {
                ...
                .priv   = &presets,
                .max    = 7,
                .choices = CHOICES("None selected", "mv1080p MCM rewire 14bit", "4K anamorphic rewired 10bit", "5K anamorphic 10bit" , "2.5K 10bit", "mv1080p 45/48/50fps 10bit", "default reset", "h264 8bit"),
