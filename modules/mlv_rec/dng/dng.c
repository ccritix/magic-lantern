...
#define HEADER_SIZE 1536 //previous value was 65536 needed by MLVFS for FUSE 64k block operations
#define COUNT(x) ((int)(sizeof(x)/sizeof((x)[0])))

#define SOFTWARE_NAME "MLVFS CDNG Writer"
 ...
 * @param frame_info The MLV blocks associated with the frame
...
size_t dng_get_header_data(struct frame_info * frame_info, uint8_t * output_buffer, off_t offset, size_t max_size)
{
    ...
    */
    ...
    if(header)
    {
        ...
        /* tcStripByteCounts */
        int flag = IMG_SIZE_MAX;
        if(frame_info->pack_dng_bits) flag = IMG_SIZE_AUTO;
        uint32_t strip_byte_counts = dng_get_image_size(frame_info, flag);
        ...
        struct directory_entry IFD0[IFD0_COUNT] =
        {
            ...
            {tcBitsPerSample,               ttShort,    1,      (!frame_info->pack_dng_bits && !frame_info->compressed_raw) ? 16 : frame_info->rawi_hdr.raw_info.bits_per_pixel},
            {tcCompression,                 ttShort,    1,      frame_info->compressed_raw == 0 ? ccUncompressed : ccJPEG},
            ...
            {tcStripByteCounts,             ttLong,     1,      strip_byte_counts},
            ...
            {tcReelName,                    ttAscii,    STRING_ENTRY(frame_info->mlv_filename, header, &data_offset)},
 ...
 */
static void dng_unpack_image_data(uint16_t * input_buffer, uint16_t * output_buffer, size_t max_size, uint32_t bpp)
{
    uint32_t pixel_end = (uint32_t)(max_size / 2);
    uint32_t mask = (1 << bpp) - 1;

    uint16_t *packed_bits = (uint16_t *)input_buffer;
    uint16_t *unpacked_bits = (uint16_t *)output_buffer;
...
/**
 * packs bits 16 bit little endian to 16 bit big endian (raw payload DNG spec)
 * input_buffer - a buffer containing the packed imaged data
 * output_buffer - the buffer where the result will be written
 * max_size - the size in bytes to write into the output_buffer
 * bpp - raw data bits per pixel
 */
static void dng_pack_image_data(uint16_t * input_buffer, uint16_t * output_buffer, size_t max_size, uint32_t bpp)
{
    uint32_t pixel_end = (uint32_t)(max_size / 2);
    ...
    for (uint32_t pixel_index = 1; pixel_index < pixel_end; pixel_index++)
    {
        ...
        16 bit words plus shift left to put the needed pixel bits to right position.
        mask high 16 bits of packed buffer 32 bit word and do logical OR to ROLed unpacked one.
...
{
    return dng_get_header_size() + dng_get_image_size(frame_info, flag);
}
...
void dng_init_data(struct frame_info * frame_info, struct dng_data * dng_data)
{
    ...
    if (first_time)
    {
        dng_data->header_size = dng_get_header_size();
        dng_data->header_buf = (uint8_t*)malloc(dng_data->header_size);
        
        /* if not compressed data pass-through mode */
        if(frame_info->compressed_raw != 2)
        {
            dng_data->image_size = dng_get_image_size(frame_info, IMG_SIZE_MAX);
            dng_data->image_buf = (uint16_t*)malloc(dng_data->image_size);
        }
        
        if(frame_info->pack_dng_bits)
        {
            dng_data->image_size_packed = dng_get_image_size(frame_info, IMG_SIZE_AUTO);
            dng_data->image_buf_packed = (uint16_t*)malloc(dng_data->image_size_packed);
        }

        /* backup the pointer to the original image buffer */
    ...
    dng_data->header_size = dng_get_header_data(frame_info, dng_data->header_buf, 0, dng_data->header_size);
    
    /* if not compressed data pass-through mode */
    if(frame_info->compressed_raw != 2)
    {
        dng_unpack_image_data(frame_info->rawi_hdr.raw_info.buffer, dng_data->image_buf, dng_data->image_size, frame_info->rawi_hdr.raw_info.bits_per_pixel);
    }
}
...
int dng_save_data(struct frame_info * frame_info, struct dng_data * dng_data)
{
    ...
    if(!frame_info->compressed_raw && frame_info->pack_dng_bits)
    {
        dng_pack_image_data(dng_data->image_buf, dng_data->image_buf_packed, dng_data->image_size, frame_info->rawi_hdr.raw_info.bits_per_pixel);

        if (fwrite(dng_data->image_buf_packed, dng_data->image_size_packed, 1, dngf) != 1)
        {
            return 0;
        }
    }
    ...
    if (frame_info->show_progress)
    {
        if (!frame_count)
        {
            switch (frame_info->compressed_raw)
            {
                case 0: 
                    printf("\nExtracting frames...\n");
                    break;
                case 2: 
                    printf("\nPassing through original lossless data...\n");
                    break;
            }
        }
        if (frame_info->compressed_raw != 1) printf("\rCurrent frame '%s' (frames saved: %d)", frame_info->dng_filename, frame_count+1);
    }
    
    /* if value is changed restore the pointer to original image buffer from backup */
...
void dng_free_data(struct frame_info * frame_info, struct dng_data * dng_data)
{
    ...
    if(frame_info->pack_dng_bits)
    {
        free(dng_data->image_buf_packed);
    }    
    free_pixel_maps();
}
