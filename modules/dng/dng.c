...
static const struct cam_matrices cam_matrices[] =
{
    ...
    {
        "Canon EOS 70D",
        { 7545, 10000, -1435, 10000, -929, 10000, -3846, 10000, 11488, 10000, 2692, 10000, -332, 10000, 1209, 10000, 6370, 10000 },
        { 7034, 10000, -804, 10000, -1014, 10000, -4420, 10000, 12564, 10000, 2058, 10000, -851, 10000, 1994, 10000, 5758, 10000 },
        { 7763, 10000, 65, 10000, 1815, 10000, 2364, 10000, 8351, 10000, -715, 10000, -59, 10000, -4228, 10000, 12538, 10000 },
        { 7464, 10000, 1044, 10000, 1135, 10000, 2648, 10000, 9173, 10000, -1820, 10000, 113, 10000, -2154, 10000, 10292, 10000 }
    },
...
static uint32_t add_timecode(double framerate, uint64_t frame, uint8_t * buffer, uint32_t * data_offset)
{
    uint32_t result = *data_offset;
...
size_t dng_write_header_data(struct dng_info * dng_info, uint8_t * header, size_t header_size)
{
    ...
    if(dng_info->xRes < dng_info->raw_info->active_area.x2 ||
       dng_info->yRes < dng_info->raw_info->active_area.y2)
    {
        dng_info->raw_info->active_area.x1 = 0;
        dng_info->raw_info->active_area.y1 = 0;
        dng_info->raw_info->active_area.x2 = dng_info->xRes;
        dng_info->raw_info->active_area.y2 = dng_info->yRes;
    }
