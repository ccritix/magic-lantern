...
#define g_mq (struct msg_queue *)MEM(0x17530)
#define g_sem (struct msg_queue *)MEM(0x17534)
//~ static struct gui_mq * mq = 0x11e00;
//~ static struct semaphore * sem = 0x11E04;
static int * obj = (int *)0x29F4; //+ 40D: 0x28CC
static int * timer_obj = (int *)0x2A28; //+ 40D:  0x28f8
	...
	while(1)
	{
		...
		if (!magic_is_off() && event_ptr->type == 0)
		{
			...
			if(event_ptr->param == BGMT_PRESS_DIRECT_PRINT) {	
				...
				int* data = 0;
				int len = 0;
				int err = prop_get_value(prop_afp_addr, &data, &len);
				
				bmp_printf(FONT_LARGE, 50, 200, "Prop data=0x%08X", data);
				bmp_printf(FONT_LARGE, 50, 250, "Prop value=0x%08X", MEM(data));	
				bmp_printf(FONT_LARGE, 50, 300, "Prop len=%d", len);
		...
		case 2:
			if( *obj == 1 // not sure
			&&  event_ptr->param != 0x10
			&&  event_ptr->param != 0x0E
			&&  event_ptr->param != 0x13
			)
				goto queue_clear;
